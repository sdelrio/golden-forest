...
make deploy         # Deploy with GIT_USER and SSH (Standard)
```

### Quality Checks & Maintenance

```bash
make check          # Run Docusaurus MDX checker (crucial for migrations)
make test           # Run unlighthouse web performance test
make build          # Production build with SSG warnings suppressed
make upgrade-all    # Upgrade all yarn dependencies
make upgrade-browserlist # Update browserslist database
```

**Note**: No unit test framework is configured. Test components by creating MDX test pages (e.g., `_test-component.mdx`) in `docs/` or `tutorial/`. Files starting with `_` are excluded from the sidebar automatically by Docusaurus.
...
src/
  components/       # React components
    ComponentName/  # Multi-file components (e.g., XmlChar, StatBlock)
      ComponentName.js
...
sidebars.js         # Sidebar configuration (auto-generated by default)
```
...
- Use **function components** with hooks (React 19 features allowed)
- Destructure props in function signature with defaults
- Use `export default` for main component, named exports for utilities
- Wrap browser-dependent components in `BrowserOnly` to avoid SSR issues
...
- Leverage Docusaurus Infima CSS variables for theming (e.g., `--ifm-color-primary`)
...
| Constants | UPPER_SNAKE_CASE | `FEATURE_LIST` |
...
- No TypeScript - use JSDoc comments for type hints (as seen in `docusaurus.config.js`)

### Error Handling

- Use state for error management in components
- Provide meaningful error messages (especially for file fetching)
...
- Include type checking or defaults for robustness
...
- Frontmatter supported in blog posts and docs
- Mermaid diagrams enabled in code blocks
- KaTeX math equations supported (via `remark-math` and `rehype-katex`)
- Table extensions supported via `rehype-extended-table`
- Test pages use `_` prefix (e.g., `_private-page.mdx`) to hide from sidebar

## Data Handling (Fantasy Grounds)

- XML character data is stored in `static/fg/chars/`
- Parsed using `DOMParser` in client-side components (see `XmlParser.js`)
- Components should handle asynchronous loading of these assets gracefully with loading states
